...
router.patch('/items/:productId', async (req: AuthRequest, res, next) => {
  try {
    ...
    const item = cart.items.find(
      (entry) => entry.productId === Number(req.params.productId),
    );
...
router.delete('/items/:productId', async (req: AuthRequest, res, next) => {
  try {
    const cart = await getCart(req.user!.id);
    const item = cart.items.find(
      (entry) => entry.productId === Number(req.params.productId),
